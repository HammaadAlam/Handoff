...
export function HomeFiltersScreen() {
  const navigation = useNavigation<NativeStackNavigationProp<HomeStackParamList>>();
  const { params } = useRoute<RouteProp<HomeStackParamList, 'Filters'>>();
  ...
  const exitFilters = useCallback(() => {
    ...
    if (filtersOnly || !navigation.canGoBack()) {
      navigation.replace('HomeLanding');
      return;
    }
    navigation.goBack();
  }, [navigation]);
  ...
  const rangeLabel = useMemo(() => {
    ...
    if (rounded >= PRICE_SLIDER_MAX) return `$0 – $${PRICE_SLIDER_MAX.toLocaleString()}+`;
  ...
  const apply = () => {
    const nextFilters: SearchFilters = { sort, priceMax, condition, sellerType, mileage };
    ...
    <SafeAreaView style={styles.root} edges={['top', 'bottom']}>
      <View style={styles.sheet}>
        ...
        <View style={styles.sheetHead}>
          ...
          <Pressable onPress={exitFilters} hitSlop={12} accessibilityLabel="Close filters">
          ...
          <View style={styles.block}>
            ...
            <View style={styles.sortChips}>
              {(
                ...
                const on = sort === key;
                return (
                  <Pressable key={key} onPress={() => setSort(key)} style={[styles.choiceChip, on && styles.choiceChipOn]}>
          ...
          <View style={styles.block}>
            ...
            <View style={[styles.chipRow, styles.chipRowTight]}>
              {CONDITIONS.map((c) => {
                ...
                return (
                  <Pressable key={c} onPress={() => setCondition((prev) => (prev === c ? null : c))} style={[styles.choiceChip, on && styles.choiceChipOn]}>
                    <Text style={[styles.choiceChipText, on && styles.choiceChipTextOn]}>{c}</Text>
                  </Pressable>
                );
              })}
            </View>
          ...
          <View style={styles.block}>
            ...
            <View style={[styles.chipRow, styles.chipRowTight]}>
              {SELLER_TYPES.map((s) => {
                ...
                return (
                  <Pressable key={s} onPress={() => setSellerType(s)} style={[styles.choiceChip, on && styles.choiceChipOn]}>
                    <Text style={[styles.choiceChipText, on && styles.choiceChipTextOn]}>{s}</Text>
                  </Pressable>
                );
              })}
            </View>
          ...
          <View style={[styles.block, styles.blockLast]}>
            ...
            <View style={styles.chipRowWrap}>
              {MILEAGE_LABELS.map((m) => {
                const on = mileage === m;
                return (
                  <Pressable key={m} onPress={() => setMileage(m)} style={[styles.choiceChip, on && styles.choiceChipOn]}>
                    <Text style={[styles.choiceChipText, on && styles.choiceChipTextOn]}>{m}</Text>
                  </Pressable>
                );
              })}
            </View>
...
const styles = StyleSheet.create({
  ...
  blockLast: { marginBottom: spacing.md },
  blockHead: {
    flexDirection: 'row',
    alignItems: 'baseline',
    justifyContent: 'space-between',
  ...
  blockTitle: { fontFamily: fonts.bold, fontSize: 16, color: colors.textPrimary },
  blockTitleGap: { marginBottom: spacing.sm },
  blockMeta: { ...typography.caption, fontSize: 13, color: colors.textMuted },
  sliderHint: {
    ...typography.caption,
  ...
  sortChips: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm },
  chipRow: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm, marginTop: spacing.sm },
  chipRowTight: { marginTop: 0 },
  ...
  choiceChipText: { fontSize: 13, fontFamily: fonts.medium, color: colors.textSecondary },
  choiceChipTextOn: { color: colors.textInverse, fontFamily: fonts.semiBold },
  footer: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.md,
  ...
  resetBtn: { paddingVertical: 12, paddingHorizontal: 4 },
  resetText: {
    ...typography.button,
    fontSize: 15,
  ...
  applyText: { ...typography.button, color: colors.textInverse, fontSize: 16 },
});
